...
export const calculatePriceWithTiers = (basePrice, quantity, pricingTiers = [], minOrderQuantity = 3, hasQuantityDiscount = false, totalCartQuantity = null) => {
  // Verwende Gesamtmenge im Warenkorb, falls vorhanden, sowohl für Mindestmengen-Check als auch für Tier-Entscheidung
  const quantityForMinCheck = totalCartQuantity !== null ? totalCartQuantity : quantity;
  const effectiveQuantity = totalCartQuantity !== null ? totalCartQuantity : quantity;
  ...
  if (effectiveQuantity >= 3 && effectiveQuantity <= 6) {
    ...
    tierName = 'Kleinmenge';
  } else if (effectiveQuantity >= 7 && effectiveQuantity < 25) {
    // Normalpreis für 7-24 SRM
    adjustmentText = 'Normalpreis';
    tierName = 'Standard';
  } else if (effectiveQuantity >= 25 && hasQuantityDiscount) {
