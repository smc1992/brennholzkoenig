...
const IMAGES = [
    {
        path: '/Users/smc/.gemini/antigravity/brain/72c7b289-03dc-459c-9092-498f2def8633/top_down_fire_1773102598402.png',
        filename: 'top_down_fire.png'
    },
    {
        path: '/Users/smc/.gemini/antigravity/brain/72c7b289-03dc-459c-9092-498f2def8633/firewood_storage_1773102614279.png',
        filename: 'firewood_storage.png'
    },
    {
        path: '/Users/smc/.gemini/antigravity/brain/72c7b289-03dc-459c-9092-498f2def8633/efficient_heating_1773102628014.png',
        filename: 'efficient_heating.png'
    }
];

const BLOG_POSTS = [
    {
        title: 'Kaminfeuer richtig anzünden: Die Top-Down-Methode im Detail',
        slug: 'kaminfeuer-richtig-anzuenden-top-down-methode',
        category: 'Ratgeber',
        tags: ['Kaminfeuer', 'Anzünden', 'Top-Down', 'Umweltschutz'],
        excerpt: 'Wie zündet man ein Feuer raucharm und umweltfreundlich an? Erfahren Sie alles über die revolutionäre Top-Down-Methode für ein perfektes Kaminfeuer ohne Qualm.',
        content_value: `
      ...
      <p>Legen Sie zunächst 2 bis 3 große, gleich dicke Holzscheite (am besten unser kammergetrocknetes Premium-Buchenholz) quer in den Brennraum. Lassen Sie dazwischen etwa 1-2 cm Abstand für die Luftzirkulation.</p>
      ...
      <p>Selbst die beste Methode scheitert an feuchtem Holz. Bei Brennholz König garantieren wir eine <strong>Restfeuchte von 6-8%</strong> durch unsere Kammertrocknung. Das bedeutet: Das Feuer entfacht sofort, schwelgt nicht und Sie haben vom ersten Moment an eine schöne, leuchtend helle Flamme.</p>
    `,
        imageIndex: 0
    },
    {
        title: 'Brennholz richtig lagern: 5 fatale Fehler, die Sie vermeiden sollten',
        slug: 'brennholz-richtig-lagern-fatale-fehler',
        category: 'Praxis-Tipps',
        tags: ['Lagerung', 'Trocknung', 'Brennholz', 'Schimmel'],
        excerpt: 'Ihr Holz schimmelt oder brennt schlecht? Entdecken Sie die 5 häufigsten Fehler bei der Holzlagerung und wie Sie mit einfachen Tricks Ihr Brennholz dauerhaft trocken halten.',
        content_value: `
      ...
      <p>Wer frisch geschlagenes Holz in Heizungskellern oder geschlossenen Garagen lagert, züchtet Schimmel. Holz braucht Wind zum Trocknen. Lagern Sie Holz nur dann im Haus, wenn es bereits komplett durchgetrocknet ist – wie unser auf 6-8% kammergetrocknetes Premiumholz vom Brennholz König, das Sie bedenkenlos sofort im Wohnzimmer neben dem Kamin stapeln können.</p>
    `,
        imageIndex: 1
    },
    {
        title: 'Heizen mit Holz: Wie Sie in diesem Winter richtig Geld sparen',
        slug: 'heizen-mit-holz-geld-sparen-effizienz',
        category: 'Ratgeber',
        tags: ['Geld sparen', 'Effizienz', 'Brennwert', 'Heizkosten'],
        excerpt: 'Wann legt man nach? Wie reguliert man die Luftzufuhr? Warum trockenes Premiumholz am Ende günstiger ist als vermeintlich billiges, nasses Holz.',
        content_value: `
      ...
      <p>Der Heizwert von Holz ist extrem abhängig von seiner Restfeuchte. Ein Beispiel: Waldfrisches Holz hat eine Feuchtigkeit von ca. 50% und einen Heizwert von nur 2,2 kWh/kg. Unser kammergetrocknetes Premium-Buchenholz mit nur 6-8% Restfeuchte erreicht hingegen über <strong>4,2 kWh/kg</strong>!</p>
      ...
      <h2>Das Fazit</h2>
      <p>Investieren Sie einmal im Jahr in hochwertiges, kammergetrocknetes Holz mit maximalem Brennwert. Wenn Sie dann noch unsere Tipps zum Top-Down-Anzünden und zur richtigen Luftregulierung beherzigen, heizen Sie nicht nur extrem klimaschonend, sondern entlasten auch spürbar Ihren Geldbeutel.</p>
    `,
        imageIndex: 2
    }
];
...
async function run() {
    console.log('🚀 Starte Blog-Erstellung (Upload & Insert)...');

    const uploadedUrls = [];

    // 1. Bilder hochladen
    for (const img of IMAGES) {
        console.log(`📦 Lade hoch: ${img.filename}...`);
        const fileBuffer = fs.readFileSync(img.path);

        // Generiere eindeutigen Namen (YYYY/MM/...)
        const uniqueFilename = `blog/${Date.now()}_${img.filename}`;
        ...
        console.log(`✅ Erfolgreich hochgeladen: ${publicUrl}`);
        uploadedUrls.push(publicUrl);
    }

    // 2. Datenbank-Einträge erstellen
    console.log('\n📝 Speichere Artikel in der Datenbank...');

    // Ermittle zuerst, wie viele Artikel es schon gibt (für sort_order)
    const { count } = await supabase
        .from('page_contents')
        .select('*', { count: 'exact', head: true })
        .eq('content_type', 'blog_post');

    let currentSortOrder = count || 0;

    for (let i = 0; i < BLOG_POSTS.length; i++) {
        const post = BLOG_POSTS[i];
        const imageUrl = uploadedUrls[post.imageIndex];
        ...
        const postData = {
            ...
            status: 'published', // Wir schalten sie direkt online!
            reading_time: 4,
            meta_title: post.title,
            meta_description: post.excerpt,
            focus_keywords: post.tags.join(', '),
            comments_enabled: true,
            content_type: 'blog_post',
            page: 'blog',
            section: 'post',
            content_key: post.slug,
            is_active: true,
        ...
        const { error } = await supabase
            .from('page_contents')
            .upsert(postData, { onConflict: 'content_key' });

        if (error) {
            console.error(`❌ Fehler beim Speichern von '${post.title}':`, error.message);
        } else {
            console.log(`✅ Artikel gespeichert: '${post.title}'`);
        }
    }
